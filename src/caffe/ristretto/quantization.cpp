...
#if QUANT_MODE
    // Input & output activations are highly likely to experience saturation unless '+1' is added. 
    // However, the author pointed out that NO '+1' (and one more fractional bit instead) showed better results even with saturation. 
    // Refer to https://groups.google.com/forum/#!topic/ristretto-users/tDveCMZI2vs
...
#else
#if QUANT_MODE
  //for (int bitwidth = 32; bitwidth > 0; bitwidth -= 1) {
  for (int bitwidth = 16; bitwidth > 0; bitwidth /= 2) {
#else
  for (int bitwidth = 16; bitwidth > 0; bitwidth /= 2) {
#endif
//#if QUANT_MODE
    EditNetDescriptionDynamicFixedPoint(&param, "Convolution_and_Deconvolution_and_PReLU", "Parameters",
//#else
//    EditNetDescriptionDynamicFixedPoint(&param, "Convolution", "Parameters",
//#endif
        bitwidth, -1, -1, -1);
    //net_test = new Net<float>(param, NULL);
    net_test = new Net<float>(param);   // Follow the latest Caffe's changes
    net_test->CopyTrainedLayersFrom(weights_);
...
#endif
//#if QUANT_MODE
    EditNetDescriptionDynamicFixedPoint(&param, "Convolution_and_Deconvolution_and_InnerProduct",
...
//#endif
  ...
  param.mutable_state()->set_phase(caffe::TEST);
//#if QUANT_MODE
  EditNetDescriptionDynamicFixedPoint(&param, "Convolution_and_Deconvolution_and_PReLU_and_InnerProduct", "Parameters_and_Activations", bw_conv_params_, bw_fc_params_, bw_in_, bw_out_);
//#else
//  EditNetDescriptionDynamicFixedPoint(&param, "Convolution_and_InnerProduct", "Parameters_and_Activations", bw_conv_params_, bw_fc_params_, bw_in_, bw_out_);
//#endif
...
//#if QUANT_MODE
    // if this is a deconvolutional layer which should be quantized ...
    if (layers_2_quantize.find("Deconvolution") != string::npos &&
        param->layer(i).type().find("Deconvolution") != string::npos) {
      // quantize parameters
      if (net_part.find("Parameters") != string::npos) {
        LayerParameter* param_layer = param->mutable_layer(i);
        param_layer->set_type("DeconvolutionRistretto");
        param_layer->mutable_quantization_param()->set_fl_params(bw_conv -
            GetIntegerLengthParams(param->layer(i).name()));
        param_layer->mutable_quantization_param()->set_bw_params(bw_conv);
      }
    ...
    // if this is a PReLU layer which should be quantized ...
    if (layers_2_quantize.find("PReLU") != string::npos &&
        param->layer(i).type().find("PReLU") != string::npos) {
      // quantize parameters
      if (net_part.find("Parameters") != string::npos) {
        LayerParameter* param_layer = param->mutable_layer(i);
        param_layer->set_type("PReLURistretto");
        param_layer->mutable_quantization_param()->set_fl_params(bw_conv -
            GetIntegerLengthParams(param->layer(i).name()));
        param_layer->mutable_quantization_param()->set_bw_params(bw_conv);
      }
